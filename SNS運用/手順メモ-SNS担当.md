...
---
### リプ欄の Reply ボタンは、打ち終わってからもう一度本文をクリックしないと出ないことがある — 2026-08-25

**状況**: `type` の後にスクロールすると、ツールバー（Reply ボタンの行）が描画されないままになる。
→ **打った文字の末尾を1回クリックして焦点を戻す → スクショを撮り直す → 実座標で押す。**
**位置は親ポストの行数で動く**（1行なら y=373、4行なら y=428、打ち終わり後は y=261〜315）。**毎回スクショで取り直す。**
**送信の確認は入力欄が空になったかで見る。** 最後に `from:kyoichi_kurashi&f=live` で実物と重複を確認する。
